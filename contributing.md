# Contributing to object-concat

Below are a few ways to make contributing to `object-concat` smoother.
...
## Setup
    ...
    git clone https://github.com/wilmoore/object-concat.js.git
    cd object-concat.js
    npm install
    npm run dev
...
If you are adding a new feature, please include tests. The test suite for this project uses [tape](https://github.com/substack/tape). To run the test suite, type `npm test`. You may also run the development watcher (`npm run dev`) which uses [nodemon](http://nodemon.io) to re-run the test suite when files are modified. You can also get the raw tap test output by running `node test`.
